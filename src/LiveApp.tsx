...
export default function LiveApp() {
  ...
  const modules: Record<string, React.ReactNode> = {"/dashboard":<Dashboard />, "/kits":<Matrix kind="kits" />, "/precificador":<Pricing />, "/contas":<Accounts />, "/usuarios":<Users />, "/anuncios":<Placeholder title="Anúncios" description="Integração com canais em preparação." />, "/pedidos":<Placeholder title="Pedidos" description="Sincronização em preparação." />, "/configuracoes":<Placeholder title="Configurações" description="Configurações em preparação." />, "/automacoes":<Placeholder title="Automações" description="Agendamentos em preparação." />};
  if(location.pathname !== "/" && location.pathname !== "/produtos") return <div className="app"><Sidebar /><main className="content rocketWorkspace"><p className="demoNotice">Este módulo ainda é demonstrativo. O catálogo real está em Produtos.</p>{modules[location.pathname] ?? <Navigate to="/produtos" replace />}</main></div>;
  return <div className="app"><Sidebar /><main className="rocketWorkspace"><div className="rocketTopbar"><img src="/brand/rocket-ia-logo.png" alt="Rocket" /><span>GESTÃO E AUTOMAÇÃO</span></div>
