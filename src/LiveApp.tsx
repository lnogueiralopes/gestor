...
export default function LiveApp() {
  ...
  if(location.pathname !== "/" && location.pathname !== "/produtos") return <div className="app"><Sidebar /><main className="content rocketWorkspace"><Breadcrumbs/>{location.pathname !== "/minha-conta" && <p className="demoNotice">Este módulo ainda é demonstrativo. O catálogo real está em Produtos.</p>}{modules[location.pathname] ?? <Navigate to="/produtos" replace />}</main></div>;
  return <div className="app"><Sidebar /><main className="rocketWorkspace"><Breadcrumbs action={formOpen?(editing?"Editar produto":"Novo produto"):undefined} onBack={()=>setFormOpen(false)}/>
    <header className="pageHeader"><ActionPanel onAdd={()=>openForm()} addLabel="Novo produto" search={search} onSearch={formOpen?undefined:setSearch}/></header>
