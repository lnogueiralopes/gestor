...
export default function LiveApp() {
  ...
  const modules: Record<string, React.ReactNode> = {"/estoque/cadastro":<Placeholder title="Cadastro de estoque" description="Identificação dos estoques por ID_Estoque. Em preparação." />, "/estoque/movimentacoes":<Placeholder title="Movimentações de estoque" description="Entradas, saídas e transferências. Em preparação." />, "/estoque/inventario":<Placeholder title="Inventário" description="Contagem e conferência de estoque. Em preparação." />,"/minha-conta":<AccountSettings />,"/dashboard":<Dashboard />, "/kits":<Matrix kind="kits" />, "/precificador":<Pricing />, "/contas":<Marketplace />, "/usuarios":<Users />, "/anuncios":<Placeholder title="Anúncios" description="Integração com canais em preparação." />, "/pedidos":<Placeholder title="Pedidos" description="Sincronização em preparação." />, "/configuracoes":<Placeholder title="Configurações" description="Configurações em preparação." />, "/automacoes":<Placeholder title="Automações" description="Agendamentos em preparação." />};
  if(location.pathname !== "/" && location.pathname !== "/produtos") return <div className="app"><Sidebar /><main className="content rocketWorkspace"><Breadcrumbs/>{location.pathname !== "/minha-conta" && location.pathname !== "/contas" && <p className="demoNotice">Este módulo ainda é demonstrativo. O catálogo real está em Produtos.</p>}{modules[location.pathname] ?? <Navigate to="/produtos" replace />}</main></div>;
