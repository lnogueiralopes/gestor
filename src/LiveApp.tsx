...
export default function LiveApp() {
  ...
  const columns=[['winery','Vinícola'],['brand','Marca'],['product_type','Tipo'],['varietal','Variedade'],['size','Tamanho']] as const;
  useEffect(() => {
...
const openForm = (product?: Product) => { setImageFields(Array.from({length:6},(_,i)=>String(product?.[`image_${i+1}_url`]??'').replace(/^\/image\//,'/images/'))); setFormOpen(true); setEditing(product ?? null); setForm({ family_id: String(product?.family_id ?? 1), ean: String(product?.ean ?? ''), sku: String(product?.sku ?? product?.ean ?? ''), id_produto: String(product?.id_produto ?? ''), name: String(product?.name ?? ''), short_title: String(product?.short_title ?? ''), brand: String(product?.brand ?? ''), origin: String(product?.origin ?? ''), winery: String(product?.winery ?? ''), vintage: String(product?.vintage ?? ''), size: String(product?.size ?? ''), varietal: String(product?.varietal ?? ''), product_type: String(product?.product_type ?? ''), summary: String(product?.summary ?? ''), description: String(product?.description ?? ''), unit_cost: String(product?.unit_cost ?? '0.00'), target_margin: String(product?.target_margin ?? '0.00'), stock_on_hand: String(product?.stock_on_hand ?? 0), quantity_per_box: String(product?.quantity_per_box ?? 1), width_cm:String(product?.width_cm ?? 7), length_cm:String(product?.length_cm ?? 7), height_cm:String(product?.height_cm ?? 28), weight_kg:String(product?.weight_kg ?? 1.1), is_active: product?.is_active !== false }); };
  ...
  const modules: Record<string, React.ReactNode> = {"/estoque/cadastro":<Placeholder title="Cadastro de estoque" description="Identificação dos estoques por ID_Estoque. Em preparação." />, "/estoque/movimentacoes":<Placeholder title="Movimentações de estoque" description="Entradas, saídas e transferências. Em preparação." />, "/estoque/inventario":<Placeholder title="Inventário" description="Contagem e conferência de estoque. Em preparação." />,"/minha-conta":<AccountSettings />,"/dashboard":<Dashboard />, "/kits":<Matrix kind="kits" />, "/precificador":<Pricing />, "/contas":<Marketplace />, "/usuarios":<RegisteredUsers />, "/anuncios":<Listings />, "/pedidos":<Placeholder title="Pedidos" description="Sincronização em preparação." />, "/configuracoes":<Placeholder title="Configurações" description="Configurações em preparação." />, "/automacoes":<Placeholder title="Automações" description="Agendamentos em preparação." />};
  if(location.pathname !== "/" && location.pathname !== "/produtos") return <div className="app"><Sidebar /><main className="content rocketWorkspace"><Breadcrumbs/>{location.pathname !== "/minha-conta" && location.pathname !== "/contas" && location.pathname !== "/usuarios" && location.pathname !== "/anuncios" && !location.pathname.startsWith('/precificador') && <p className="demoNotice">Este módulo ainda é demonstrativo. O catálogo real está em Produtos.</p>}{modules[location.pathname] ?? (location.pathname.startsWith('/precificador') ? <Pricing /> : <Navigate to="/produtos" replace />)}</main></div>;
