...
function Pricing() {
  ...
  useEffect(()=>{if(supabase)supabase.from('pricing_logs').select('*').order('created_at',{ascending:false}).limit(50).then(({data})=>{if(data)setPricingLogs(data);});},[]);
  ...
  return (
    <>
      ...
      {section==='log' && <div className="card tableWrap"><div className="sectionHeading"><label className="logPeriod">Período<select value={logPeriod} onChange={e=>setLogPeriod(e.target.value)}><option value="7">Últimos 7 dias</option><option value="30">Últimos 30 dias</option><option value="90">Últimos 90 dias</option><option value="all">Todo o histórico</option></select></label></div><table><thead><tr><th>Data</th><th>Usuário</th><th>Ação</th><th>Detalhes</th></tr></thead><tbody>{pricingLogs.length?pricingLogs.map(log=><tr key={log.id}><td>{new Date(log.created_at).toLocaleString('pt-BR')}</td><td>{log.user_id||'Usuário atual'}</td><td>{log.action} ({log.affected_count} itens)</td><td><button type="button" className="infoButton" title="Baixar detalhes em TXT" onClick={()=>{const blob=new Blob([JSON.stringify(log.details,null,2)],{type:'text/plain;charset=utf-8'});const u=URL.createObjectURL(blob);const a=document.createElement('a');a.href=u;a.download=`log-${log.id}.txt`;a.click();setTimeout(()=>URL.revokeObjectURL(u),500);}}>⇩</button></td></tr>):<tr><td>—</td><td>—</td><td>Nenhuma alteração registrada</td><td>Os próximos uploads e cadastros aparecerão aqui.</td></tr>}</tbody></table></div>}
      ...
      {section==='tariffs' && <div className="marketplaceRuleGroups">{Array.from(new Set(feeRows.map(r=>r.channel))).map(channel=><div className="card tableWrap marketplaceRuleGroup" key={channel}><div className="ruleIntro"><h3>{channel}</h3><p className="muted">Comissões e taxas fixas consideradas no preço final de cada venda.</p></div><table><thead><tr><th>Família</th><th>Modalidade</th><th>Faixa</th><th>Comissão</th><th>Taxa fixa</th></tr></thead><tbody>{feeRows.filter(r=>r.channel===channel).map(r=><tr key={r.id}><td>{r.product_families?.name||'Todas'}</td><td>{r.listing_type||'Padrão'}</td><td>{r.max_price?`R$ ${Number(r.min_price||0).toFixed(2)} a R$ ${Number(r.max_price).toFixed(2)}`:`A partir de R$ ${Number(r.min_price||0).toFixed(2)}`}</td><td>{Number(r.commission_percent||0).toFixed(2)}%</td><td>R$ {Number(r.fixed_fee||0).toFixed(2).replace('.',',')}</td></tr>)}</tbody></table>{!feeRows.filter(r=>r.channel===channel).length&&<p className="muted">Nenhuma regra cadastrada.</p>}</div>)}{!feeRows.length&&<div className="card"><p className="muted">Carregando as premissas de tarifas…</p></div>}</div>}
