...
function Pricing() {
  ...
  return (
    <>
      ...
      {section==='log' && <div className="card tableWrap"><div className="sectionHeading"><label className="logPeriod">Período<select value={logPeriod} onChange={e=>setLogPeriod(e.target.value)}><option value="7">Últimos 7 dias</option><option value="30">Últimos 30 dias</option><option value="90">Últimos 90 dias</option><option value="all">Todo o histórico</option></select></label></div><table><thead><tr><th>Data</th><th>Usuário</th><th>Ação</th><th>Detalhes</th></tr></thead><tbody>{pricingLogs.length?pricingLogs.map(log=><tr key={log.id}><td>{new Date(log.created_at).toLocaleString('pt-BR')}</td><td>{logUserNames[log.user_id]||'Usuário atual'}</td><td>{log.action} ({log.affected_count} itens)</td><td><button type="button" className="infoButton" title="Baixar detalhes em TXT" onClick={()=>{const blob=new Blob([JSON.stringify(log.details,null,2)],{type:'text/plain;charset=utf-8'});const u=URL.createObjectURL(blob);const a=document.createElement('a');a.href=u;a.download=`log-${log.id}.txt`;a.click();setTimeout(()=>URL.revokeObjectURL(u),500);}}>⇩</button></td></tr>):<tr><td>—</td><td>—</td><td>Nenhuma alteração registrada</td><td>Os próximos uploads e cadastros aparecerão aqui.</td></tr>}</tbody></table></div>}
      ...
      {section==='matrix' && <div className="card tableWrap"><div className="matrixLegend"><span><b>1</b> ML Clássico Padrão</span><span><b>2</b> ML Premium Padrão</span><span><b>3</b> ML Clássico Campanha +10%</span><span><b>4</b> ML Premium Campanha +10%</span><span><b>5</b> Shopee Padrão</span><span><b>6</b> Shopee Campanha +10%</span></div><table><thead><tr><th>EAN</th><th>Produto</th><th>Família</th><th>Custo</th><th>Margem</th><th>1</th><th>2</th><th>3</th><th>4</th><th>5</th><th>6</th></tr></thead><tbody>{Array.from(new Map(matrixRows.map(row=>[row.id,row])).values()).map(product=>{const rows=matrixRows.filter(row=>row.id===product.id);const price=(name:string)=>rows.find(row=>row.pricing_table_name===name)?.calculated_price;return <tr key={product.id}><td>{product.ean}</td><td>{product.name}</td><td>{product.family_name}</td><td>R$ {Number(product.unit_cost||0).toFixed(2).replace('.',',')}</td><td>{Number(product.target_margin||0).toFixed(2).replace('.',',')}%</td>{['Preço Clássico','Preço Premium','Clássico + 10% campanha','Premium + 10% campanha','Preço normal','Normal + 10% campanha'].map(name=><td key={name} title={price(name)!=null ? (name + " — Custo R$ " + Number(product.unit_cost).toFixed(2) + " | Dimensões " + product.width_cm + "×" + product.length_cm + "×" + product.height_cm + " cm | Peso " + product.weight_kg + " kg + " | Margem " + product.target_margin + "% | Comissão " + product.commission_percent + "% | Taxa fixa R$ " + Number(product.commission_fixed_fee).toFixed(2) + " | Frete R$ " + Number(product.freight_value).toFixed(2) + " | Lucro líquido R$ " + (Number(price(name))-Number(product.unit_cost)-(Number(price(name))*Number(product.commission_percent)/100)-Number(product.commission_fixed_fee)-Number(product.freight_value)).toFixed(2)) : "Preço indisponível"}>{price(name)!=null?'R$ '+Number(price(name)).toFixed(2).replace('.',','):'—'}</td>)}</tr>})}</tbody></table>{!matrixRows.length&&<p className="muted">Nenhum produto calculado.</p>}</div>}
