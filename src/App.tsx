...
function Pricing() {
  ...
  return (
    <>
      ...
      {section==='matrix' && <div className="card tableWrap"><div className="matrixLegend"><span><b>1</b> ML Clássico Padrão</span><span><b>2</b> ML Premium Padrão</span><span><b>3</b> ML Clássico Campanha +10%</span><span><b>4</b> ML Premium Campanha +10%</span></div><table><thead><tr><th>EAN</th><th>Produto</th><th>Família</th><th>1</th><th>2</th><th>3</th><th>4</th></tr></thead><tbody>{Array.from(new Map(matrixRows.map(row=>[row.id,row])).values()).map(product=>{const rows=matrixRows.filter(row=>row.id===product.id);const price=(name:string)=>rows.find(row=>row.pricing_table_name===name)?.calculated_price;return <tr key={product.id}><td>{product.ean}</td><td>{product.name}</td><td>{product.family_name}</td>{['Preço Clássico','Preço Premium','Clássico + 10% campanha','Premium + 10% campanha'].map(name=><td key={name}>{price(name)!=null?'R$ '+Number(price(name)).toFixed(2).replace('.',','):'—'}</td>)}</tr>})}</tbody></table>{!matrixRows.length&&<p className="muted">Nenhum produto calculado.</p>}</div>}
