...
function Pricing() {
  ...
  return (
    <>
      ...
      {section==='matrix'&&recalcBusy&&recalcJob?.scope?.mode==='general'&&<div className="card" aria-live="polite"><progress max={recalcJob.scope.total||1} value={recalcJob.scope.processed||0}/><p>{recalcJob.scope.processed} de {recalcJob.scope.total} produtos calculados.</p></div>}
      {section==='matrix'&&recalcStatus&&<div className="card" role="alert"><p>{recalcStatus}</p><button onClick={()=>setRecalcStatus('')}>Dispensar</button></div>}
