...
function Pricing() {
  ...
  return (
    <>
      ...
      {section==='matrix' && <div className="card tableWrap"><table><thead><tr><th>EAN</th><th>Produto</th><th>Família</th><th>Marketplace</th><th>Tabela</th><th>Preço calculado</th></tr></thead><tbody>{matrixRows.map(row=><tr key={`${row.id}-${row.pricing_table_id}`}><td>{row.ean}</td><td>{row.name}</td><td>{row.family_name}</td><td>{row.channel}</td><td>{row.pricing_table_name}</td><td title={`Custo R$ ${row.unit_cost}; margem ${row.target_margin}%; comissão ${row.commission_percent}%; frete R$ ${row.freight_value}`}>R$ {Number(row.calculated_price).toFixed(2).replace('.',',')}</td></tr>)}</tbody></table>{!matrixRows.length&&<p className="muted">Nenhum produto calculado.</p>}</div>}
