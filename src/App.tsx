...
function Pricing() {
  ...
  const downloadCostModel = async () => {
    ...
    const csv = [headers,...rows.map((p:any)=>[`="${p.ean||''}"`,`="${p.sku||p.ean||''}"`,p.name||'',familyName(p.family_id),Number(p.unit_cost??p.cost??0).toFixed(2).replace('.',','),Number(p.target_margin??p.margin??0).toFixed(2).replace('.',','),'',''])].map(row=>row.map((v:any)=>String(v).replace(/\t|\r?\n/g,' ')).join(';')).join('\r\n');
