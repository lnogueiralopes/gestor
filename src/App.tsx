...
function Pricing() {
  ...
  return (
    <>
      ...
      {section==='matrix' && <div className="card tableWrap"><div className="matrixLegend"><span><b>1</b> ML Clássico Padrão</span><span><b>2</b> ML Premium Padrão</span><span><b>3</b> ML Clássico Campanha +10%</span><span><b>4</b> ML Premium Campanha +10%</span><span><b>5</b> Shopee Padrão</span><span><b>6</b> Shopee Campanha +10%</span></div><table><thead><tr><th>EAN</th><th>Produto</th><th>Família</th><th>Custo</th><th>Margem</th><th>1</th><th>2</th><th>3</th><th>4</th><th>5</th><th>6</th></tr></thead><tbody>{Array.from(new Map(matrixRows.map(row=>[row.id,row])).values()).map(product=>{const rows=matrixRows.filter(row=>row.id===product.id);const price=(name:string)=>{const campaign=/campanha/i.test(name);const channel=(/shopee|normal/i.test(name)&&!/(clássico|premium)/i.test(name))?'Shopee':'Mercado Livre';return rows.find(row=>row.channel===channel&&((campaign&&Number(row.adjustment_percent)>0)||(!campaign&&!Number(row.adjustment_percent))))?.calculated_price;};return <tr key={product.id}><td>{product.ean}</td><td>{product.name}</td><td>{product.family_name}</td><td>{Number(product.unit_cost||0).toFixed(2).replace('.',',')}</td><td>{Number(product.target_margin||0).toFixed(2).replace('.',',')}%</td>{['Preço Clássico','Preço Premium','Clássico + 10% campanha','Premium + 10% campanha','Preço normal','Normal + 10% campanha'].map(name=><td key={name} onClick={()=>setCalcDetail(`Tabela: ${name} | Custo: R$ ${Number(product.unit_cost||0).toFixed(2)} | Margem: ${Number(product.target_margin||0)}% | Dimensões: ${product.width_cm||'—'} × ${product.length_cm||'—'} × ${product.height_cm||'—'} cm | Peso: ${product.weight_kg||'—'} kg | Frete e comissões conforme premissas cadastradas`)} title={price(name)!=null ? ("Custo R$ " + Number(product.unit_cost).toFixed(2) + " | Margem " + product.target_margin + "% | Comissão " + product.commission_percent + "% | Taxa fixa R$ " + Number(product.commission_fixed_fee).toFixed(2) + " | Frete R$ " + Number(product.freight_value).toFixed(2) + " | Lucro líquido R$ " + (Number(price(name))-Number(product.unit_cost)-(Number(price(name))*Number(product.commission_percent)/100)-Number(product.commission_fixed_fee)-Number(product.freight_value)).toFixed(2)) : "Preço indisponível"}>{price(name)!=null?Number(price(name)).toFixed(2).replace('.',','):'—'}</td>)}</tr>})}</tbody></table>{!matrixRows.length&&<p className="muted">Nenhum produto calculado.</p>}</div>}{calcDetail&&<div className="calcModalBackdrop" role="presentation" onClick={()=>setCalcDetail('')}><div className="calcModal" role="dialog" aria-modal="true" onClick={e=>e.stopPropagation()}><button type="button" className="calcModalClose" onClick={()=>setCalcDetail('')}>×</button><h3>Memória de cálculo</h3><p>{calcDetail}</p></div></div>}
