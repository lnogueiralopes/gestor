...
export default function FreightGroups(){
  ...
  const quote=(g:Row)=>operation(g.id,async()=>{
    ...
    const result=await res.json();if(!res.ok)throw new Error(result.error);update(result);
  });
  ...
  return <div className="card tableWrap freightGroups"><h3>Grupos logísticos / Fretes</h3>
    ...
    <table><thead><tr>{['Grupo','Família','Unidades / modelo','SKUs','Peso (kg)','Dimensões de referência (cm)','Volume (L)','Frete ML','Origem / atualização','Cotação de referência','Ações'].map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>
    {rows.filter(g=>!pendingOnly||g.freight_origin==='pending'||!g.confirmed_at).map(g=><tr key={g.id} className={g.freight_origin==='pending'?'pendingFreight':''}>
      ...
      <td><input aria-label={'Preço de referência do grupo '+g.group_number} placeholder="Preço do item" inputMode="decimal" value={prices[g.id]??String(g.quote_price??'')} onChange={e=>setPrices({...prices,[g.id]:e.target.value})}/><small>{g.quote_listing_type==='gold_pro'?'Premium':'Clássico'} / {g.quote_logistic_type}</small>{g.pending_quote&&<div>Cotação API: {money(g.pending_quote.freight_net_value)}<small>{new Date(g.pending_quote_at).toLocaleString('pt-BR')}</small><button className="roundAction" title="Usar cotação" aria-label={"Usar cotação do grupo "+g.group_number} disabled={busy.includes(g.id)} onClick={()=>setAccept(g)}>✓</button></div>}</td>
      <td><button className="roundAction" disabled={busy.includes(g.id)} title="Salvar frete manual" aria-label={'Salvar frete do grupo '+g.group_number} onClick={()=>void save(g)}>✓</button><button className="roundAction" disabled={busy.includes(g.id)} title="Calcular frete" aria-label={'Calcular frete do grupo '+g.group_number} onClick={()=>void quote(g)}>↻</button></td>
    </tr>)}</tbody></table>
